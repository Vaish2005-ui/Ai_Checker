...
// ── GitHub Integration ────────────────────────────────────────────────────────
export async function syncGitHub(companyId: string, githubUsername: string) {
  const res = await fetch(`${BASE}/department/${companyId}/software/github-sync`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ company_id: companyId, github_username: githubUsername }),
  });
  return res.json();
}

export const getGitHubAnalysis = (companyId: string) =>
  get(`/department/${companyId}/software/github-analysis`);
