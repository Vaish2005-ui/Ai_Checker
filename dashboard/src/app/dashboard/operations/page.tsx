...
import { Target, TrendingUp, Package, Truck, AlertTriangle, BarChart3, Activity, Zap, Clock, CheckCircle2, ArrowUpRight, Boxes, Workflow, Gauge } from "lucide-react";
...
export default function OperationsDashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/department/${compId}/operations/risk`).then(r => r.json())
  ...
  const giants = metrics["Giants"] ?? 0;
  const poorFit = metrics["Poor Market Fit"] ?? 0;
  const acquisition = metrics["Acquisition Stagnation"] ?? 0;
  const competition = metrics["Competition"] ?? 0;

  const efficiency = (85 - giants * 15 - poorFit * 10).toFixed(0);
  const slaCompliance = (98 - competition * 8 - acquisition * 5).toFixed(1);
  const throughput = Math.round(450 - giants * 100 - poorFit * 80);
  const processMaturity = (70 - acquisition * 20 - competition * 10).toFixed(0);
  ...
  return (
    <div className="pb-24 animate-fade-in">
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-xl bg-rose-50 border border-rose-100 flex items-center justify-center"><Target className="w-6 h-6 text-rose-500" /></div>
            <h1 className="text-4xl font-bold text-slate-900 tracking-tight">Operations</h1>
            <span className="text-[10px] px-2.5 py-1 rounded-full bg-slate-900 text-white font-bold tracking-wider">ML-POWERED</span>
          </div>
          <p className="text-slate-500 text-sm">Efficiency, SLAs, process maturity &amp; competitive positioning — powered by ML predictions.</p>
        </div>
        <div className={`${CARD} px-6 py-4 flex items-center gap-5`}>
          <div className="relative w-16 h-16">
            <svg className="w-16 h-16 -rotate-90" viewBox="0 0 36 36">
              <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#f1f5f9" strokeWidth="3" />
              <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke={healthColor} strokeWidth="3" strokeDasharray={`${Number(efficiency)}, 100`} strokeLinecap="round" className="transition-all duration-1000" />
            </svg>
            <div className="absolute inset-0 flex items-center justify-center"><span className="text-xl font-black text-slate-900">{efficiency}</span></div>
          </div>
          <div>
            <p className="text-[10px] text-slate-400 uppercase tracking-wider font-bold">Efficiency</p>
            <p className="text-sm font-black" style={{ color: healthColor }}>{Number(efficiency) >= 70 ? "Optimal" : Number(efficiency) >= 50 ? "Suboptimal" : "Poor"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        {[
          { label: "Efficiency", value: `${efficiency}%`, icon: Gauge, color: "#22c55e", sub: "Operational output", bg: "bg-green-50/50" },
          { label: "SLA Compliance", value: `${slaCompliance}%`, icon: CheckCircle2, color: "#3b82f6", sub: "Target: 99%+", bg: "bg-blue-50/50" },
          { label: "Throughput", value: `${throughput}`, icon: ArrowUpRight, color: "#8b5cf6", sub: "Units per week", bg: "bg-violet-50/50" },
          { label: "Process Maturity", value: `${processMaturity}%`, icon: Workflow, color: "#f59e0b", sub: "CMMI Level", bg: "bg-amber-50/50" },
        ].map((m, i) => (
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        <div className={`${CARD} p-8`}>
          <h3 className="text-base font-bold text-slate-900 flex items-center gap-2 mb-6">
            <div className="w-8 h-8 rounded-full bg-rose-50 flex items-center justify-center"><Boxes className="w-4 h-4 text-rose-500" /></div>
            Operational Metrics
          </h3>
          <div className="space-y-6">
            {[
              { label: "Output Efficiency", value: `${efficiency}%`, bar: Number(efficiency), color: "#22c55e", icon: "⚡" },
              { label: "Quality Score", value: `${(90 - poorFit * 30).toFixed(0)}%`, bar: 90 - poorFit * 30, color: "#3b82f6", icon: "✅" },
              { label: "Resource Util.", value: `${(75 - giants * 20).toFixed(0)}%`, bar: 75 - giants * 20, color: "#8b5cf6", icon: "📊" },
              { label: "Cycle Time", value: `${(5 + acquisition * 8).toFixed(1)}d`, bar: Math.min(100, (5 + acquisition * 8) * 5), color: "#f59e0b", icon: "⏱️" },
            ].map((item, i) => (
              <div key={i}>
        ...
        <div className="space-y-6">
          {[
            { title: "Market Fit Risk", val: poorFit, icon: Package, iconBg: "bg-amber-50", iconColor: "text-amber-500" },
            { title: "Competition Impact", val: competition, icon: Truck, iconBg: "bg-red-50", iconColor: "text-red-500" },
          ].map((gauge, i) => (
        ...
        <div className={`${CARD} p-8`}>
          ...
          <div className="space-y-4">
            {[
              { label: "SLA Compliance", value: `${slaCompliance}%`, unit: "current", icon: CheckCircle2, color: "#22c55e" },
              { label: "Giants Pressure", value: `${(giants * 100).toFixed(0)}%`, unit: "impact", icon: TrendingUp, color: "#ef4444" },
              { label: "Throughput", value: `${throughput}`, unit: "units/wk", icon: BarChart3, color: "#3b82f6" },
      ...
      <div className={`${CARD} p-6 flex items-start gap-4 bg-slate-900 text-white`}>
        <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center flex-shrink-0"><Zap className="w-5 h-5 text-rose-400" /></div>
        <div>
          <p className="text-base font-bold mb-1">ML Model Integration</p>
          <p className="text-sm text-slate-300 leading-relaxed font-medium">
            Operations metrics from: <code className="text-rose-400 bg-slate-800 px-1.5 py-0.5 rounded">Giants</code>, <code className="text-rose-400 bg-slate-800 px-1.5 py-0.5 rounded">Poor Market Fit</code>, <code className="text-rose-400 bg-slate-800 px-1.5 py-0.5 rounded">Acquisition Stagnation</code>, and <code className="text-rose-400 bg-slate-800 px-1.5 py-0.5 rounded">Competition</code>.
          </p>
        </div>
      </div>
    </div>
