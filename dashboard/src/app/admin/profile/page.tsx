...
export default function StartupProfilePage() {
  ...
  const handleGenerateAnalysis = async () => {
    ...
    try {
      ...
      const finalReport = {
        company: "Company Overview", // Or fetch company name from backend
        sector: "Global",
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto pb-24">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Startup Profile</h1>
          <p className="text-slate-400">Manage global company parameters that influence overall risk scores.</p>
        </div>
        <div className="flex gap-3">
          <button 
            onClick={handleUpdate}
            disabled={saving || generating}
            className="flex items-center gap-2 px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-medium transition-colors disabled:opacity-50"
          >
      ...
      <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4 mb-8 flex items-start gap-4">
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-sm font-bold text-indigo-400 tabular-nums">{value ?? 0}</span>
      </div>
      <input 
        type="range"
