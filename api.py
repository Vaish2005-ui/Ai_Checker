...
from github_analyzer import analyze_github_account
...
def software_dev_metrics(company_id: str):
    ...
    return {
        ...
        "github_username": comp.get("github_username", ""),
    }

# ══════════════════════════════════════════════════════════════════════════════
# GITHUB INTEGRATION — Auto-sync from GitHub account
# ══════════════════════════════════════════════════════════════════════════════
...
@app.post("/department/{company_id}/software/github-sync")
def github_sync(company_id: str, req: GitHubSyncRequest):
    """
    Fetch real data from GitHub's public API, analyze it,
    derive DORA metrics, and store them as the software department metrics.
    """
    comp = get_company(company_id)
    if not comp:
        raise HTTPException(status_code=404, detail="Company not found")
    ...
    analysis = analyze_github_account(req.github_username)
    if "error" in analysis:
        raise HTTPException(status_code=400, detail=analysis["error"])
    
    # Extract normalized metrics for the ML model
    model_m = analysis.get("model_metrics", {})
    
    # Update department metrics with real GitHub-derived values
    if "department_metrics" not in comp:
        comp["department_metrics"] = {}
    comp["department_metrics"]["software"] = {
        "Tech Debt": model_m.get("tech_debt", 0.2),
        "Security Risk": model_m.get("security_risk", 0.2),
        "change_failure_rate": model_m.get("change_failure_rate", 0.1),
        "deployment_frequency": model_m.get("deployment_frequency", 0.5),
        "lead_time_days": model_m.get("lead_time_days", 14.0),
        "mttr_hours": model_m.get("mttr_hours", 24.0),
    }
    
    # Store the GitHub username and last analysis result
    comp["github_username"] = req.github_username
    comp["github_analysis"] = {
        "username": analysis["username"],
        "analyzed_at": analysis["analyzed_at"],
        "repo_count": analysis["repo_count"],
        "dora": analysis["dora"],
        "github_activity": analysis["github_activity"],
        "code_health": analysis["code_health"],
        "tech_debt_score": analysis["tech_debt_score"],
        "security_risk_score": analysis["security_risk_score"],
        "raw_stats": analysis["raw_stats"],
        "languages": analysis["languages"],
        "repo_details": analysis["repo_details"],
        "sprint_velocity": analysis["sprint_velocity"],
        "open_bugs": analysis["open_bugs"],
        "resolved_this_week": analysis["resolved_this_week"],
    }
    
    save_company(comp)
    logger.info("GitHub sync complete for %s → %s", company_id, req.github_username)
    
    # Return the full analysis + updated devmetrics
    return {
        "status": "synced",
        "github_username": req.github_username,
        **comp["github_analysis"],
        "overall_risk": _compute_software_risk(comp),
    }


@app.get("/department/{company_id}/software/github-analysis")
def get_github_analysis(company_id: str):
    """Return the cached GitHub analysis for the software department."""
    comp = get_company(company_id)
    if not comp:
        raise HTTPException(status_code=404, detail="Company not found")
    
    analysis = comp.get("github_analysis")
    if not analysis:
        return {"connected": False, "github_username": ""}
...
def _compute_software_risk(comp: dict) -> float:
    """Helper to compute the software department risk using the ML model."""
    global_m = comp.get("global_metrics", {})
    dept_m = comp.get("department_metrics", {}).get("software", {})
    merged = {**global_m, **dept_m}
    base = to_dict(StartupProfile())
